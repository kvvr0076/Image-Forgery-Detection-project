...
def predict():
    ...
    test = test/255
    preds = shufflenet.predict(test)
    predict = np.argmax(preds)
    img = cv2.imread(filename)
    img = cv2.resize(img, (800,400))
    cv2.putText(img, 'image Recognize as : '+labels[predict], (10, 25),  cv2.FONT_HERSHEY_SIMPLEX,0.7, (0, 255, 0), 2)
    cv2.imshow('image Recognize as : '+labels[predict], img)
    cv2.waitKey(0)
    
    
def graph():
    da = pd.DataFrame([['SqueezeNet','Precision',precision[0]],['SqueezeNet','Recall',recall[0]],['SqueezeNet','F1 Score',fscore[0]],['SqueezeNet','Accuracy',accuracy[0]],
    ...
    da.pivot("Parameters", "Algorithms", "Value").plot(kind='bar')
    plt.show()
...
siftsvmButton = Button(main, text="Run Existing SIFT Model", command=siftSVM)
